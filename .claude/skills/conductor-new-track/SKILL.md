...
PLAN MODE PROTOCOL: Parts of this process run within Plan Mode. While in Plan Mode, you are explicitly permitted and required to use the Write tool, the Edit tool, and authorized the Bash tool calls to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by Plan Mode security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in the Bash tool calls while in Plan Mode and will cause tool failure.**
...
2.  **Get Track Description & Enter Plan Mode:**
    *   **If `$ARGUMENTS` is empty:**
        1. Call the Plan Mode tool with the reason: "Defining new track".
    ...
    *   **If `$ARGUMENTS` contains a description:**
        1. Use the content of `$ARGUMENTS` as the track description.
        2. Call the Plan Mode tool with the reason: "Defining new track".
...
5.  **Write Files:**
    ...
    *   Write the index file to `<Tracks Directory>/<track_id>/index.md` with content:
        ...
        ```
6.  **Exit Plan Mode:** Call the exit Plan Mode tool with the path: `<Tracks Directory>/<track_id>/index.md`.
