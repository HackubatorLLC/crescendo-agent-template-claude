...
PLAN MODE PROTOCOL: This setup process runs entirely within Plan Mode. While in Plan Mode, you are explicitly permitted and required to use the Write tool, the Edit tool, and authorized the Bash tool calls to create and modify files within the `conductor/` directory. **CRITICAL: You MUST use relative paths starting with `conductor/` (e.g., `conductor/product.md`) for all file operations. Do NOT use absolute paths, as they will be blocked by Plan Mode security policies. REDIRECTION (e.g., `>` or `>>`) is strictly NOT allowed in the Bash tool calls while in Plan Mode and will cause tool failure.** Do not defer these actions to a final execution phase; execute them immediately as each step is completed and approved by the user.
---
...
1.  **Enter Plan Mode:** Call the Plan Mode tool with the reason: "Setting up Conductor project".
...
2.  **Select Code Style Guides:**
    -   List the available style guides by using the the Bash tool tool to execute `ls conductor/templates/code_styleguides/`. **CRITICAL: You MUST use the Bash tool for this step. Do NOT use the `list_directory` tool, as the templates directory resides outside of your allowed workspace and the call will fail.**
...
3.  **Action:** Construct and execute a command to create the directory and copy all selected files. For example: `mkdir -p conductor/code_styleguides && cp conductor/templates/code_styleguides/python.md conductor/templates/code_styleguides/javascript.md conductor/code_styleguides/`
4.  **Continue:** Immediately proceed to the next section.

### 2.5 Select Workflow (Interactive)
1.  **Copy Initial Workflow:**
    -   Copy `conductor/templates/workflow.md` to `conductor/workflow.md`.
2.  **Determine Mode:** Use the AskUserQuestion tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Workflow"
...
3.  **Generate Track Artifacts:**
    ...
    d. **Exit Plan Mode:** Call the exit Plan Mode tool with the path: `<Tracks Directory>/<track_id>/index.md`.
